...
### Несколько инстансов API и worker (отказоустойчивость)

Состояние приложения в PostgreSQL / Redis / MongoDB по-прежнему **один узел** на тип сервиса в этом `docker-compose.yml` — отказоустойчивость **горизонтального масштабирования** относится к **безсостоятельным** процессам **API** и **worker**.

В `docker-compose.yml` задано по умолчанию:

- `api`: **`scale: 3`** — три реплики REST API;
- `worker`: **`scale: 2`** — два потребителя очереди BullMQ.

Балансировка по round-robin на стороне **nginx** в контейнере `web`: в шаблоне включены Docker DNS `127.0.0.11` и `proxy_pass` через переменную, чтобы при нескольких IP у имени `api` запросы не залипали на одном инстансе.

Число реплик при запуске можно переопределить (имеет приоритет над полем `scale` в файле):

```bash
docker compose up -d --scale api=1 --scale worker=1
```

Контейнер `web` ждёт успешный **healthcheck** сервиса `api` (`GET /api/health`), поэтому nginx стартует после готовности **всех** реплик API.

Для полной отказоустойчивости БД и брокера нужны уже **кластеры** PostgreSQL / Redis (или управляемые сервисы), это выходит за рамки текущего compose.

Сервисы:

| Сервис | Порт | Описание |
|--------|------|----------|
| `api` | 3000 | REST API (можно масштабировать `--scale api=N`) |
| `web` | 80/443 | Nginx: web SPA + admin SPA + API proxy |
| `worker` | — | Обработчик очередей (можно `--scale worker=N`) |
